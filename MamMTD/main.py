...
def parse_args():
    ...
    parser.add_argument('--input_dim', type=int, default=204, help='网络输入的维度 UNSW-NB15 204 10 CICIDS2017 78 14 Nba-IoT 115 10')
    ...
    # parser.add_argument('--communication_round', type=int, default=5, help='通讯轮数')
...
if __name__ == '__main__':
    ...
    if args.task == 'multi':
        if args.dataset == 'UNSW-NB15':
            X_train, X_test, y_train, y_test, attack_encode = preprocess_data_UNSW15("D:\\Chenhaolei_experiment\\dataset\\UNSW-NB15\\CSV Files", args)
            # X_train, X_test, y_train, y_test, acttack_encode = preprocess_data_UNSW15("/home/chl/dataset/11", args)
        elif args.dataset == 'CICIDS2017':
            X_train, X_test, y_train, y_test, attack_encode = preprocess_data_CICIDS("D:\\Chenhaolei_experiment\\dataset\\CICIDS2017\\MachineLearningCVE", args)
        elif args.dataset == 'Nba-IoT':
            X_train, X_test, y_train, y_test, attack_encode = preprocess_data_NbaIoT("D:\\Chenhaolei_experiment\\dataset\\N_BaIOT\\archive", args)
        else:
            print("数据集不存在 ")
